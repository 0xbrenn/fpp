...
const totalValue = filteredAssets.reduce((sum, a) => {
  try {
    const price = ethers.BigNumber.from(a.pricePerShare || 0);
    const total = ethers.BigNumber.from(a.totalShares || 0);
...
const totalFractions = filteredAssets.reduce((sum, a) => {
  try {
    return sum.add(ethers.BigNumber.from(a.totalShares || 0));
  } catch (err) {
    return sum;
  }
}, ethers.BigNumber.from(0));
...
const totalVolume = filteredAssets.reduce((sum, a) => {
  try {
    return sum.add(ethers.BigNumber.from(a.totalRevenue || 0));
  } catch (err) {
    return sum;
  }
}, ethers.BigNumber.from(0));
  ...
  {parseFloat(ethers.utils.formatEther(totalValue)).toLocaleString('en-US', { 
    ...
    maximumFractionDigits: 2 
            ...
            <div className="bg-black/50 backdrop-blur-sm p-3 sm:p-4 md:p-6 border" style={{borderColor: 'rgba(34, 128, 205, 0.3)'}}>
              ...
              <p className="text-lg sm:text-xl md:text-2xl font-light text-white">
  {parseFloat(ethers.utils.formatEther(totalFractions)).toLocaleString('en-US', { 
