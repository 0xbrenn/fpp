...
// ✅ FIXED: Only shows listings user can actually purchase (or their own)
...
const SecondaryMarketplace = () => {
    ...
    try {
      ...
      for (const assetId of assetIds) {
        try {
          ...
          // ✅ Also check if user has reached max positions limit
          const positionCount = positionIds.length;
          const maxPositions = asset.maxPositionsPerUser || asset[15] || 0;
  ...
  // ✅ IMPROVED: Check if user can purchase a specific listing
  const canUserPurchase = useCallback((listing) => {
    ...
    // ✅ CHECK #1: Max positions per user reached
    if (holdings.maxPositions > 0 && holdings.positionCount >= holdings.maxPositions) {
      return {
        canPurchase: false,
        reason: `Max positions reached: You have ${holdings.positionCount} positions (limit: ${holdings.maxPositions} per user)`
      };
    }
    
    // ✅ CHECK #2: Max purchase amount limit
    // No limit set
  ...
  // ✅ CRITICAL FIX: Apply filters and search + HIDE UNPURCHASEABLE LISTINGS
  const filteredAndSortedListings = useMemo(() => {
    let filtered = [...listings];

    // ✅ FIRST: Filter out listings where user can't purchase (except their own)
    if (address && !showMyListings) {
      filtered = filtered.filter(l => {
        const isMyListing = l.seller.toLowerCase() === address.toLowerCase();
        if (isMyListing) return true; // Always show user's own listings
        
        const { canPurchase } = canUserPurchase(l);
        return canPurchase; // Only show listings user can actually purchase
      });
    }
    ...
    return filtered;
  }, [listings, searchQuery, filterAssetType, filterOwnership, filterModel, showMyListings, priceRange, sortBy, address, canUserPurchase]);
...
<div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4">
  <div className="bg-black/50 backdrop-blur-sm border border-neutral-800 rounded-lg p-2.5 sm:p-6 min-w-0">
    ...
    <p className="text-2xl sm:text-2xl font-light text-white mb-0.5 text-center sm:text-left">{filteredAndSortedListings.length}</p>
    <p className="text-[8px] sm:text-xs text-green-500 text-center sm:text-left">Available</p>
  </div>
       ...
       {/* Premium Filter Bar */}
        <div className="bg-neutral-950 border-b border-neutral-900 sticky top-0 z-20 backdrop-blur-sm bg-opacity-95">
          <div className="px-3 sm:px-6 lg:px-8 py-3 sm:py-4">
            <div className="flex flex-col gap-3 sm:gap-4">
              ...
              {/* Filter Controls - MOBILE HORIZONTAL SCROLL - FIXED */}
              <div className="-mx-3 sm:mx-0 px-3 sm:px-0">
                <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-2 sm:flex-wrap sm:overflow-visible">
                  {/* Asset Type */}
                  <select
                    value={filterAssetType}
                    onChange={(e) => setFilterAssetType(e.target.value)}
                    className="flex-shrink-0 min-w-[100px] sm:min-w-[120px] px-2.5 sm:px-4 py-2 sm:py-3 bg-black border border-neutral-800 rounded-xl text-white text-xs sm:text-sm focus:outline-none focus:border-blue-500 transition-all"
                  >
                  ...
                  <div className="flex-shrink-0 flex bg-black border border-neutral-800 rounded-xl overflow-hidden">
                    <button
                      ...
                      className={`px-2.5 sm:px-4 py-2 sm:py-3 transition-all ${
                        viewMode === 'grid'
                          ? 'bg-white text-black'
                  ...
                  <button
                    ...
                    className={`flex-shrink-0 px-2.5 sm:px-4 py-2 sm:py-3 border rounded-xl transition-all flex items-center gap-1.5 text-xs sm:text-sm ${
        ...
        <div className="px-3 sm:px-6 lg:px-8 py-2 sm:py-3 bg-black/50 border-b border-neutral-900 flex flex-col sm:flex-row items-center justify-between gap-2">
          <p className="text-xs sm:text-sm text-neutral-400 text-center sm:text-left">
            Found <span className="text-white font-medium">{filteredAndSortedListings.length}</span> listings you can purchase
        ...
        <div className="px-3 sm:px-6 lg:px-8 py-6 sm:py-8">
          {filteredAndSortedListings.length === 0 ? (
            <div className="text-center py-20 sm:py-32 px-4">
              ...
              <h3 className="text-xl sm:text-2xl font-light text-white mb-2">No listings available</h3>
              <p className="text-sm sm:text-base text-neutral-500 mb-6 sm:mb-8 max-w-md mx-auto">
                {activeFilterCount > 0 
                  ? 'Try adjusting your filters to see more results' 
                  : showMyListings
                  ? 'You don\'t have any active listings yet'
                  : 'No listings you can purchase right now. You may have reached position limits on available assets.'}
              </p>
              {activeFilterCount > 0 && (
                <button
                  onClick={clearFilters}
            ...
            // List View
            // IMPROVED LIST VIEW - Mobile Responsive for SecondaryMarketplace.jsx
// Replace the list view section with this
    ...
    return (
      ...
      >
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6">
          {/* Image - Clean on both mobile and desktop */}
          ...
          <div className="flex-1 w-full space-y-3">
            {/* Asset Info */}
            <div>
              <p className="text-[10px] sm:text-xs text-neutral-500 uppercase tracking-wider mb-1">
                {listing.assetType}
              </p>
              <h4 className="font-medium text-white text-base sm:text-lg mb-2">
                {listing.assetName}
              </h4>
              
              {/* Badges - Below title */}
              <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-2">
                <span className="px-2 py-0.5 bg-neutral-800 text-white text-[10px] sm:text-xs rounded-md">
                  P2P
                </span>
                {listing.isWeighted && (
                  <span className="px-2 py-0.5 bg-purple-600/20 text-purple-400 text-[10px] sm:text-xs rounded-md">
                    Weighted
                  </span>
                )}
                {listing.isPartialSale && (
                  <span className="px-2 py-0.5 bg-blue-600/20 text-blue-400 text-[10px] sm:text-xs rounded-md">
                    Partial
                  </span>
                )}
                {isMyListing && (
                  <span className="px-2 py-0.5 bg-green-600/20 text-green-400 text-[10px] sm:text-xs rounded-md">
                    Your Listing
                  </span>
                )}
              </div>

              {/* Seller - Mobile compact */}
              <p className="text-[10px] sm:text-xs text-neutral-500">
                Seller: <span className="font-mono text-neutral-400">{listing.seller.slice(0, 6)}...{listing.seller.slice(-4)}</span>
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-base sm:text-xl font-light text-white">
                  {listing.displayPercentage}
                </p>
                {!listing.isWeighted && (
                  <p className="text-[10px] sm:text-xs text-neutral-500">{listing.displayAmount}</p>
                )}
              </div>
            ...
            <button
              ...
              className="w-full sm:w-auto px-6 py-2.5 sm:py-3 rounded-xl font-medium transition-all flex items-center justify-center gap-2 text-sm bg-blue-600 hover:bg-blue-700 text-white transform hover:-translate-y-0.5"
            >
              {isMyListing ? (
                <>
                  <Eye className="w-3 h-3 sm:w-4 sm:h-4" />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-gradient-to-b from-neutral-900 to-black border border-neutral-800 rounded-2xl max-w-lg w-full shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-4 sm:p-6">
              ...
              {/* Limit Warning - Only show if user can't purchase and it's NOT their listing */}
              {(() => {
                ...
                return !canPurchase && selectedListing.seller.toLowerCase() !== address?.toLowerCase() && (
                  <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-4 mb-4 sm:mb-6">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-yellow-500 mb-1">Cannot Purchase</p>
