    ...
    if (address) {
      try {
        ...
        for (const positionId of userPositions) {
          ...
          // ✅ FIX: Convert wei units to percentage correctly
          // position.amount is in wei units where 1e18 = 100%
          const amountWei = position.amount;
          const ownershipPercent = parseFloat(ethers.utils.formatEther(amountWei)) * 100; // Convert to %
          
          const totalValue = assetData.totalValue 
            ? parseFloat(ethers.utils.formatEther(assetData.totalValue))
            : 0;
          
          // Calculate purchase value based on percentage
