...
const AssetDetailView = ({ asset, onBack }) => {
  ...
  // Calculate metrics - FIXED VERSION: Handle both WEIGHTED and FIXED models
  ...
  if (isWeighted) {
    ...
    soldPercentage = (soldWeight / 100); // basis points to percentage
    ...
    if (asset.totalShares) {
      totalShares = ethers.BigNumber.isBigNumber(asset.totalShares)
        ? parseFloat(ethers.utils.formatEther(asset.totalShares))
        : parseFloat(asset.totalShares);
    }
    
    if (asset.availableShares) {
      availableShares = ethers.BigNumber.isBigNumber(asset.availableShares)
        ? parseFloat(ethers.utils.formatEther(asset.availableShares))
    ...
    <div className="min-h-screen bg-black">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
              ...
              <div className="hidden sm:block bg-black/60 backdrop-blur-sm p-6 rounded-sm">
                <div className="grid grid-cols-2 gap-6">
                  <div>
                    <p className="text-xs font-light text-neutral-400 mb-1">{displayStats.label1}</p>
                    <p className="text-xl font-semibold text-white">{displayStats.value1}</p>
                  </div>
                  <div>
                    <p className="text-xs font-light text-neutral-400 mb-1">{displayStats.label2}</p>
                    <p className="text-xl font-semibold text-white">{displayStats.value2}</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div className="sm:hidden px-4 py-4 bg-neutral-950 border-b border-neutral-900">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <p className="text-xs font-light text-neutral-400 mb-1">{displayStats.label1}</p>
            <p className="text-lg font-semibold text-white">{displayStats.value1}</p>
          </div>
          <div>
            <p className="text-xs font-light text-neutral-400 mb-1">{displayStats.label2}</p>
            <p className="text-lg font-semibold text-white">{displayStats.value2}</p>
          </div>
        </div>
      </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-6">
                      <div className="bg-neutral-950 border border-neutral-900 p-4">
                        <p className="text-xs font-light text-neutral-500 mb-2">{displayStats.pricePer1Label}</p>
              ...
              {activeTab === 'details' && (
                <div className="space-y-6">
                  <div className="grid grid-cols-2 gap-6">
                    ...
                    <div>
                      <p className="text-xs font-light text-neutral-500 mb-2">Total Value</p>
                      <p className="text-white font-normal">{displayStats.value1}</p>
                    </div>
                    <div>
                      <p className="text-xs font-light text-neutral-500 mb-2">Available</p>
                      <p className="text-white font-normal">{displayStats.value2}</p>
                    </div>
                    <div>
            ...
            <div className="lg:sticky lg:top-8 bg-black border border-neutral-900 rounded-sm">
              <div className="p-6 border-b border-neutral-900">
                ...
                <div className="flex justify-between text-sm">
                  <div>
                    <p className="text-neutral-500 font-light">{displayStats.soldLabel}</p>
                    <p className="text-white font-normal">{displayStats.soldValue}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-neutral-500 font-light">{displayStats.availableLabel}</p>
                    <p className="text-white font-normal">{displayStats.availableValue}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6 space-y-4">
                <div>
                  <p className="text-xs font-light text-neutral-500 mb-1">{displayStats.pricePer1Label}</p>
