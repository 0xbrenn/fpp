...
const AssetDetailView = ({ asset, onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-black">
      {/* Premium Header */}
  {/* Premium Header */}
<div className="bg-gradient-to-b from-neutral-950 to-black border-b border-neutral-900 relative z-50">
  ...
  <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-6 lg:p-8">
    <div className="max-w-[1920px] mx-auto">
      <div className="flex items-center gap-2 mb-2 sm:mb-3 flex-wrap">
        ...
        <span className="px-2 sm:px-3 py-1 bg-green-600/80 backdrop-blur-sm rounded-lg text-[10px] sm:text-xs text-white flex items-center gap-1">
        ...
        {isWeighted ? (
          ...
          <span className="px-2 sm:px-3 py-1 bg-blue-600/80 backdrop-blur-sm rounded-lg text-[10px] sm:text-xs text-white">
            Fixed
          </span>
        )}
      </div>
      <h1 className="text-xl sm:text-2xl lg:text-3xl font-light text-white mb-2">{asset.assetName}</h1>
      <div className="flex items-center gap-2 text-neutral-300">
        {vehicleData ? <Car className="w-3 h-3 sm:w-4 sm:h-4" /> : <MapPin className="w-3 h-3 sm:w-4 sm:h-4" />}
        <span className="text-xs sm:text-sm font-light">{getLocationDisplay()}</span>
      </div>
    </div>
  </div>
