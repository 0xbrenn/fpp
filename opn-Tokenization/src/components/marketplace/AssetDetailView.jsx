...
const AssetDetailView = ({ asset, onBack }) => {
  ...
  // Calculate metrics
  // Calculate metrics - FIX: Convert BigNumbers properly
const totalShares = asset.totalShares 
  ? parseFloat(ethers.utils.formatEther(asset.totalShares)) 
  : 0;
const availableShares = asset.availableShares 
...
const availablePercentage = totalShares > 0 ? (availableShares / totalShares) * 100 : 0;
  ...
  <p className="text-white font-normal">{formatNumber(totalShares)}</p>
</div>
<div>
  <p className="text-xs font-light text-neutral-500 mb-2">Available Shares</p>
  <p className="text-white font-normal">{formatNumber(availableShares)}</p>
</div>
                    <div>
  ...
  <p className="text-2xl font-semibold text-white">{formatNumber(pricePerShare)} OPN</p>
</div>
