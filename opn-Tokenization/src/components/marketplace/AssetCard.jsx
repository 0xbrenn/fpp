...
// FIXED - Consistent card heights with proper text handling
...
const AssetCard = ({ asset, onBuyClick, viewMode = 'grid' }) => {
  ...
  const availability = (() => {
    if (isWeighted) {
      const availablePercent = asset.availableWeight ? asset.availableWeight / 100 : 0;
      const soldPercent = asset.soldWeight ? asset.soldWeight / 100 : 0;
      return {
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="w-full md:w-48 h-32 bg-neutral-900 rounded-lg overflow-hidden flex-shrink-0 relative">
            ...
            <div className="absolute top-2 left-2 flex gap-2">
              <span className="px-2 py-1 bg-neutral-800/90 backdrop-blur-sm text-xs font-normal text-white rounded-sm">
                {asset.assetType}
              </span>
            </div>
            
            <div className="absolute top-2 right-2">
              <span className={`px-2 py-1 backdrop-blur-sm text-xs font-normal rounded-sm ${
                isWeighted 
    ...
    >
      ...
      <div className="aspect-video bg-neutral-900 overflow-hidden relative flex-shrink-0">
        ...
        <div className="absolute top-3 left-3">
          <span className="px-3 py-1 bg-neutral-800/90 backdrop-blur-sm text-xs font-normal text-white rounded-sm">
            {asset.assetType}
          </span>
        </div>
        
        <div className="absolute top-3 right-3 flex gap-2">
          <span className="px-2 py-1 bg-green-500/20 backdrop-blur-sm text-xs font-normal text-green-400 border border-green-500/30 rounded-sm flex items-center gap-1">
            <Check className="w-3 h-3" />
            Verified
          </span>
          <span className={`px-3 py-1 backdrop-blur-sm text-xs font-normal rounded-sm ${
            isWeighted 
              ? 'bg-green-500/20 text-green-400 border border-green-500/30' 
              : 'bg-blue-500/20 text-blue-400 border border-blue-500/30'
          }`}>
