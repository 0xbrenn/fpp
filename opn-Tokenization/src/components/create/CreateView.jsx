...
// ENHANCED with Partner Support - Tonino Lamborghini & Mansory
...
const CreateView = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    // PARTNER FIELDS - NEW
    partner: '',
    partnerAssetCategory: '',
    partnerBedrooms: '',
    partnerFloor: '',
    partnerView: '',
    partnerVehicleCategory: '',
    partnerProductionNumber: '',
  ...
  const validateStep = (step) => {
    ...
    switch (step) {
      case 1:
        ...
        // Partner validation
        if (formData.partner === 'tonino-lamborghini' && !formData.partnerBedrooms) {
          newErrors.partnerBedrooms = 'Bedroom configuration required for Tonino Lamborghini residences';
        }
        
        if (formData.partner === 'mansory' && !formData.partnerAssetCategory) {
          newErrors.partnerAssetCategory = 'Asset category required for Mansory collection';
        }
        
        if (formData.partner === 'mansory' && formData.partnerAssetCategory === 'apartment' && !formData.partnerBedrooms) {
          newErrors.partnerBedrooms = 'Bedroom configuration required for Mansory residences';
        }
        
        if (formData.partner === 'mansory' && formData.partnerAssetCategory === 'vehicle' && !formData.partnerVehicleCategory) {
          newErrors.partnerVehicleCategory = 'Vehicle category required for Mansory vehicles';
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // BUILD PARTNER METADATA
      const partnerMetadata = {};
      
      if (formData.partner) {
        partnerMetadata.partner = formData.partner;
        
        if (formData.partner === 'tonino-lamborghini') {
          partnerMetadata.assetCategory = 'apartment';
          if (formData.partnerBedrooms) partnerMetadata.bedrooms = formData.partnerBedrooms;
          if (formData.partnerFloor) partnerMetadata.floor = formData.partnerFloor;
          if (formData.partnerView) partnerMetadata.view = formData.partnerView;
          if (formData.propertySize) partnerMetadata.squareFootage = formData.propertySize;
        }
        
        if (formData.partner === 'mansory') {
          if (formData.partnerAssetCategory) {
            partnerMetadata.assetCategory = formData.partnerAssetCategory;
            
            if (formData.partnerAssetCategory === 'apartment') {
              if (formData.partnerBedrooms) partnerMetadata.bedrooms = formData.partnerBedrooms;
              if (formData.partnerFloor) partnerMetadata.floor = formData.partnerFloor;
              if (formData.partnerView) partnerMetadata.view = formData.partnerView;
              if (formData.propertySize) partnerMetadata.squareFootage = formData.propertySize;
            ...
            if (formData.partnerAssetCategory === 'vehicle') {
              if (formData.partnerVehicleCategory) {
                partnerMetadata.vehicleCategory = formData.partnerVehicleCategory;
              }
              if (formData.partnerProductionNumber) {
                partnerMetadata.productionNumber = formData.partnerProductionNumber;
              }
              if (formData.vehicleYear) partnerMetadata.year = formData.vehicleYear;
              if (formData.vehicleMake) partnerMetadata.make = formData.vehicleMake;
              if (formData.vehicleModel) partnerMetadata.model = formData.vehicleModel;
            }
          }
        }
      }
      
      const metadataString = Object.keys(partnerMetadata).length > 0 ? JSON.stringify(partnerMetadata) : '';
  ...
  // Render step 1: Asset Details (ENHANCED WITH PARTNER SUPPORT)
    ...
    <div className="space-y-6">
      <div className="bg-neutral-900 p-6 border border-neutral-800">
        ...
        <div className="space-y-4">
          ...
          {/* ============ PARTNER SELECTION SECTION - NEW ============ */}
          <div className="mt-6 pt-6 border-t border-neutral-800">
            <div className="flex items-center gap-2 mb-4">
              <Crown className="w-5 h-5 text-yellow-500" />
              <h4 className="text-lg font-normal text-white">Partner Association</h4>
              <span className="ml-auto px-2 py-1 text-xs bg-yellow-500/20 text-yellow-400 rounded border border-yellow-500/30">
                Optional
              </span>
            </div>
            <p className="text-xs text-neutral-500 mb-4">
              Associate this asset with one of our featured luxury partners. Partner assets receive special showcase placement.
            </p>

            <div>
              <label className="block text-sm text-neutral-400 mb-2">Select Partner</label>
              <select
                value={formData.partner}
                onChange={(e) => {
                  handleInputChange('partner', e.target.value);
                  handleInputChange('partnerAssetCategory', '');
                  handleInputChange('partnerBedrooms', '');
                  handleInputChange('partnerFloor', '');
                  handleInputChange('partnerView', '');
                  handleInputChange('partnerVehicleCategory', '');
                  handleInputChange('partnerProductionNumber', '');
                  
                  // Auto-set Asset Type based on partner
                  if (e.target.value === 'tonino-lamborghini') {
                    handleInputChange('assetType', 'Real Estate');
                  }
                  // For Mansory, let user choose category first
                }}
                className="w-full bg-black border border-neutral-800 px-4 py-3 text-white"
              >
                <option value="">No Partner Association</option>
                <option value="tonino-lamborghini">🏢 Tonino Lamborghini Residences</option>
                <option value="mansory">👑 Mansory Residences</option>
              </select>
              {formData.partner && (
                <p className="text-xs mt-2 p-2 rounded" style={{
                  backgroundColor: formData.partner === 'tonino-lamborghini' ? 'rgba(200, 16, 46, 0.1)' : 'rgba(212, 175, 55, 0.1)',
                  color: formData.partner === 'tonino-lamborghini' ? '#C8102E' : '#D4AF37',
                  border: `1px solid ${formData.partner === 'tonino-lamborghini' ? 'rgba(200, 16, 46, 0.3)' : 'rgba(212, 175, 55, 0.3)'}`
                }}>
                  ✓ This asset will be featured on the {formData.partner === 'tonino-lamborghini' ? 'Tonino Lamborghini' : 'Mansory'} partner page
                </p>
              )}
            </div>

            {/* TONINO LAMBORGHINI SPECIFIC FIELDS */}
            {formData.partner === 'tonino-lamborghini' && (
              <div className="mt-4 p-4 rounded border" style={{
                backgroundColor: 'rgba(200, 16, 46, 0.05)',
                borderColor: 'rgba(200, 16, 46, 0.3)'
              }}>
                <h5 className="text-sm font-semibold mb-3" style={{ color: '#C8102E' }}>
                  Tonino Lamborghini Residence Details
                </h5>
                
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm text-neutral-400 mb-2">Bedroom Configuration *</label>
                    <select
                      value={formData.partnerBedrooms}
                      onChange={(e) => handleInputChange('partnerBedrooms', e.target.value)}
                      className={`w-full bg-black border px-4 py-3 text-white ${
                        errors.partnerBedrooms ? 'border-red-500' : 'border-neutral-800'
                      }`}
                    >
                    ...
                    {errors.partnerBedrooms && (
                      <p className="text-red-400 text-xs mt-1">{errors.partnerBedrooms}</p>
                    )}
                  </div>

                  <div>
                    <label className="block text-sm text-neutral-400 mb-2">Floor Number</label>
                    <input
                      type="text"
                      value={formData.partnerFloor}
                      onChange={(e) => handleInputChange('partnerFloor', e.target.value)}
                      placeholder="e.g., 15, Top Floor, Penthouse"
                      className="w-full bg-black border border-neutral-800 px-4 py-3 text-white"
                    />
                  </div>

                  <div>
                    <label className="block text-sm text-neutral-400 mb-2">View Type</label>
                    <input
                      type="text"
                      value={formData.partnerView}
                      onChange={(e) => handleInputChange('partnerView', e.target.value)}
                ...
                <div className="mt-3 p-2 bg-black/30 rounded border border-neutral-800">
                  <p className="text-xs text-neutral-400">
                    💡 These details will be prominently displayed on the Tonino Lamborghini partner page
                  </p>
                </div>
              </div>
            )}

            {/* MANSORY SPECIFIC FIELDS */}
            {formData.partner === 'mansory' && (
              <div className="mt-4 p-4 rounded border" style={{
                backgroundColor: 'rgba(212, 175, 55, 0.05)',
                borderColor: 'rgba(212, 175, 55, 0.3)'
              }}>
                <h5 className="text-sm font-semibold mb-3" style={{ color: '#D4AF37' }}>
                  Mansory Collection Details
                </h5>

                <div className="space-y-4">
                  <div>
                    <label className="block text-sm text-neutral-400 mb-2">Asset Category *</label>
                    <select
                      value={formData.partnerAssetCategory}
                      onChange={(e) => {
                        handleInputChange('partnerAssetCategory', e.target.value);
                        handleInputChange('partnerBedrooms', '');
                        handleInputChange('partnerVehicleCategory', '');
                        
                        // Auto-sync Asset Type with partner category
                        if (e.target.value === 'apartment') {
                          handleInputChange('assetType', 'Real Estate');
                        } else if (e.target.value === 'vehicle') {
                          handleInputChange('assetType', 'Vehicles');
                        }
                      }}
                      className={`w-full bg-black border px-4 py-3 text-white ${
                        errors.partnerAssetCategory ? 'border-red-500' : 'border-neutral-800'
                      }`}
                    >
                      <option value="">Select Category</option>
                      <option value="apartment">🏢 Residence/Apartment</option>
                      <option value="vehicle">🏎️ Vehicle</option>
                    </select>
                    {errors.partnerAssetCategory && (
                      <p className="text-red-400 text-xs mt-1">{errors.partnerAssetCategory}</p>
                    )}
                  </div>

                  {/* APARTMENT FIELDS */}
                  {formData.partnerAssetCategory === 'apartment' && (
                    <>
                      <div>
                        <label className="block text-sm text-neutral-400 mb-2">Bedroom Configuration *</label>
                        <select
                      ...
                      <div>
                        ...
                        <input
                          ...
                          value={formData.partnerFloor}
                          onChange={(e) => handleInputChange('partnerFloor', e.target.value)}
                          placeholder="e.g., 20, Top Floor, Penthouse"
                          className="w-full bg-black border border-neutral-800 px-4 py-3 text-white"
                        />
                      </div>
                  ...
                  {/* VEHICLE FIELDS */}
                  {formData.partnerAssetCategory === 'vehicle' && (
                    <>
                      <div>
                        <label className="block text-sm text-neutral-400 mb-2">Vehicle Category *</label>
                        <select
                          value={formData.partnerVehicleCategory}
                          onChange={(e) => handleInputChange('partnerVehicleCategory', e.target.value)}
                          className={`w-full bg-black border px-4 py-3 text-white ${
                            errors.partnerVehicleCategory ? 'border-red-500' : 'border-neutral-800'
                          }`}
                        >
                          <option value="">Select Category</option>
                          <option value="supercar">🏎️ Supercar</option>
                          <option value="luxury-suv">🚙 Luxury SUV</option>
                          <option value="hypercar">⚡ Hypercar</option>
                          <option value="limited-edition">💎 Limited Edition</option>
                        </select>
                        {errors.partnerVehicleCategory && (
                          <p className="text-red-400 text-xs mt-1">{errors.partnerVehicleCategory}</p>
                        )}
                      </div>

                      <div>
                        <label className="block text-sm text-neutral-400 mb-2">Production Number</label>
                        <input
                          type="text"
                          value={formData.partnerProductionNumber}
                          onChange={(e) => handleInputChange('partnerProductionNumber', e.target.value)}
                          placeholder="e.g., 003/100, 1 of 50"
                          className="w-full bg-black border border-neutral-800 px-4 py-3 text-white"
                        />
                        <p className="text-xs text-neutral-500 mt-1">
                          For limited edition vehicles, specify the unit number
                        </p>
                      </div>
                    </>
                  )}
                ...
                <div className="mt-3 p-2 bg-black/30 rounded border border-neutral-800">
                  <p className="text-xs text-neutral-400">
                    💡 These details will be prominently displayed on the Mansory partner page
                  </p>
                </div>
              </div>
            )}
          ...
          {/* Only show Real Estate fields if NOT a Mansory vehicle */}
          {formData.assetType === 'Real Estate' && !(formData.partner === 'mansory' && formData.partnerAssetCategory === 'vehicle') && (
            <>
              <div>
          ...
          {/* Only show Vehicle fields if NOT a partner apartment */}
          {formData.assetType === 'Vehicles' && 
           formData.partner !== 'tonino-lamborghini' && 
           !(formData.partner === 'mansory' && formData.partnerAssetCategory === 'apartment') && (
            <>
            ...
            {formData.partner && (
              <div className="flex justify-between py-2 border-b border-neutral-800">
                ...
                <span className="text-white font-semibold" style={{
                  ...
                  {formData.partner === 'tonino-lamborghini' ? 'Tonino Lamborghini' : 'Mansory'}
                </span>
              </div>
            )}
