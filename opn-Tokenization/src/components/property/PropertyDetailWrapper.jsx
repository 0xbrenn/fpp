...
// ✅ CORRECTED - Asset ID 0 is VALID, only check creator address
...
const PropertyDetailWrapper = () => {
  ...
  useEffect(() => {
    const fetchAsset = async () => {
      // ✅ Allow asset ID 0 - it's valid! Only check if registry exists
      if (!assetRegistry || assetId === undefined || assetId === null) {
      ...
      try {
        ...
        // ✅ ONLY check creator address to see if asset exists
        // Asset ID 0 is VALID - just check if it has been created
        if (!asset || asset.creator === ethers.constants.AddressZero) {
          console.log('Asset not found - creator is zero address');
        ...
        console.log('Asset model:', asset.model, 'isWeighted:', isWeighted);
        ...
        console.log('Built base asset:', baseAsset);
  ...
  const handleBack = () => {
    navigate('/realestate'); // Navigate to property list
  };

  // Handle purchase success - THIS WAS THE REAL PROBLEM
  const handlePurchaseSuccess = async () => {
    console.log('Purchase successful! Refreshing data...');
    
    // Option 1: Reload the page to get fresh data
    window.location.reload();
    
    // Option 2: Navigate back to list (cleaner UX)
    // navigate('/realestate');
  };
