...
const PropertyModal = ({ property, onClose, onPurchaseSuccess }) => {
  ...
  // ✅ FIX: Fetch user's current shares properly for FIXED model
useEffect(() => {
  const fetchUserShares = async () => {
    ...
    try {
      ...
      // ✅ FIX: Sum up all position amounts for this asset
      let totalShares = 0;
      for (const posId of positionIds) {
        const position = await positionNFT.positions(posId);
        const amount = position.amount || position[3];
      ...
      setUserCurrentShares(totalShares);
...
const getValidationWarning = () => {
  ...
  if (!isNaN(numValue) && numValue > 0) {
    if (isWeightedModel) {
      const maxPct = getMaxAllowedAmount();  // ✅ Use calculated max
      const minPct = propertyData.minWeight || 0.001;
      const clampedValue = Math.min(maxPct, Math.max(minPct, numValue));
      setPercentage(clampedValue);
      setCustomInput(clampedValue.toString());  // ✅ Update display to clamped value
    } else {
      const maxShares = getMaxAllowedAmount();  // ✅ Use calculated max
      const minShares = propertyData.minPurchaseAmount || 1;
      const clampedValue = Math.min(maxShares, Math.max(minShares, Math.floor(numValue)));
      setShareCount(clampedValue);
      setCustomInput(clampedValue.toString());  // ✅ Update display to clamped value
    }
  } else {
    setCustomInput(value);  // Allow empty or invalid input temporarily
  }
};
