...
const PortfolioView = () => {
  ...
  // FIXED: Use new contracts
  const { assetRegistry, positionNFT, primaryMarket, governance } = useContract();
  ...
  // FIXED: Completely rewritten for new contract architecture
  const fetchUserData = useCallback(async () => {
    ...
    try {
      ...
      const holdingsPromises = assetIds.map(async (assetId) => {
        try {
          // Get user's positions for this asset
          ...
          for (const posId of positionIds) {
            ...
            totalAmount = totalAmount.add(position.amount || position[3]);
          }
          ...
          // Calculate ownership percentage based on model
          let percentageOwned = 0;
          if ((asset.model || asset[7]) === 0) {
            // FIXED model - percentage of total supply
            ...
            percentageOwned = totalAmount.toNumber() / 100; // Convert basis points to percentage
          }
          
          // Calculate current value
          let currentValue;
          if ((asset.model || asset[7]) === 0) {
            // FIXED: tokens * price per token
            const pricePerToken = asset.pricePerToken || asset[9];
            currentValue = ethers.utils.formatEther(pricePerToken.mul(totalAmount));
          } else {
            // WEIGHTED: (basis points / 10000) * total value
            const totalValue = asset.totalValue || asset[11];
            const weight = totalAmount.toNumber() / 10000;
          ...
          return {
            ...
            assetType: asset.assetType || asset[2] || '',
            assetImageUrl: asset.mainImageUrl || asset[5] || '',
            creator: asset.creator || asset[1],
            location: "Global",
            pricePerShare: (asset.model || asset[7]) === 0 
              ? ethers.utils.formatEther(asset.pricePerToken || asset[9] || '0')
              : ethers.utils.formatEther((asset.totalValue || asset[11] || ethers.BigNumber.from(0)).div(10000)),
            totalValue: currentValue,
            percentageOwned,
            purchaseValue: parseFloat(currentValue) * 0.85, // Estimate
            gain: parseFloat(currentValue) * 0.15, // Estimate
            gainPercentage: 17.65 // Estimate
          };
        } catch (err) {
  ...
  // FIXED: Fetch proposals from Governance contract
  const fetchProposals = useCallback(async () => {
    ...
    try {
      ...
      for (const asset of userAssets) {
        try {
          // Get proposal IDs for this asset
          ...
          if (proposalIds && proposalIds.length > 0) {
            ...
            for (const proposalId of proposalIds) {
              try {
                ...
                console.error(`Error fetching proposal ${proposalId}:`, propError);
              }
            }
  ...
  // FIXED: Handle proposal creation
  ...
  // FIXED: Listen for events from correct contracts
  useEffect(() => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {userAssets.length === 0 ? (
                ...
                userAssets.map((asset) => {
                  ...
                  const activeCount = assetProposals.filter(p => p.statusText === 'active').length;
                      ...
                      <div className="bg-black border border-neutral-900 overflow-hidden group hover:border-neutral-800 transition-all duration-500">
                        ...
                        <div className="p-6">
                          ...
                          <div className="space-y-2 pt-4 border-t border-neutral-900">
                            ...
                            <div className="flex justify-between text-sm">
                              <span className="text-neutral-500 font-light">Positions Held</span>
                              <span className="text-white font-normal">{asset.positionCount}</span>
                            </div>
                          </div>
                          ...
                          <div className="flex gap-2 mt-4 pt-4 border-t border-neutral-900">
                            ...
                            <button
                              onClick={() => {
                                window.open(`https://explorer.cor3innovations.io/address/${assetRegistry?.address}`, '_blank');
                              }}
