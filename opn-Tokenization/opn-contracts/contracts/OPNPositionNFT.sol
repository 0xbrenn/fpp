...
contract OPNPositionNFT is ERC721Enumerable, AccessControl, ReentrancyGuard {
    ...
    struct Position {
        ...
        uint256 assetId;
        address owner;
        uint256 amount; // tokens or basis points
        uint256 purchasePrice; // avg purchase price
    ...
    function splitPosition(uint256 _positionId, uint256 _amountToSplit, address _recipient) 
        external onlyRole(MINTER_ROLE) returns (uint256) 
    {
        ...
        positions[newPositionId] = Position({
            ...
            purchasePrice: originalPos.purchasePrice,
            purchaseTimestamp: block.timestamp,
            ethRewardsClaimed: 0,
            usdcRewardsClaimed: 0,
            lastClaimTimestamp: 0
    ...
    // =========================================================================
    // REWARD CLAIMING - CLEAN, NO COMMAS!
    // =========================================================================
    ...
    function _claimRewards(uint256 _positionId, bool _revertOnZero) private {
        ...
        if (model == IOPNAssetRegistry.PurchaseModel.FIXED) {
            ...
            // WEIGHTED: Use soldWeight as denominator
            uint256 divisor = soldWeight > 0 ? soldWeight : 10000;
            ethReward = (ethPool * pos.amount) / divisor;
            usdcReward = (usdcPool * pos.amount) / divisor;
        }
