...
interface IOPNAssetRegistry {
    ...
    // ✅ NEW: Update secondary market price
...
/**
 * @title OPNSecondaryMarket - FINAL VERSION WITH PRICE UPDATES
 * @notice P2P marketplace for trading position NFTs
 * @dev Updates asset registry prices based on sales
 */
contract OPNSecondaryMarket is AccessControl, ReentrancyGuard, Pausable {
    ...
    function buyPosition(uint256 _listingId) external payable nonReentrant whenNotPaused {
        ...
        // ✅ NEW: Update asset registry price based on this sale
        _updateAssetPrice(listing.assetId, listing.price, listing.amountListed);
        
        // Transfer payments using call() for better compatibility
        if (fee > 0) {
    ...
    // =========================================================================
    // ✅ NEW: PRICE UPDATE LOGIC
    // =========================================================================
    
    /**
     * @notice Update asset price in registry based on this sale
     * @dev Calculates price per unit and updates registry
     */
    function _updateAssetPrice(uint256 _assetId, uint256 _salePrice, uint256 _amount) private {
        try assetRegistry.getAssetModel(_assetId) returns (IOPNAssetRegistry.PurchaseModel model) {
            uint256 newPrice;
            
            if (model == IOPNAssetRegistry.PurchaseModel.WEIGHTED) {
                // For WEIGHTED: calculate total value from this sale
                // _amount is in wei units (1e18 = 100%)
                // Example: If 5% sold for 50 OPN, then 100% = 1000 OPN
                
                if (_amount > 0) {
                    // Calculate price for 100% based on this sale
                    // newPrice = (_salePrice * 1e18) / _amount
                    newPrice = (_salePrice * 1e18) / _amount;
                }
            } else {
                // For FIXED: calculate price per token
                if (_amount > 0) {
                    newPrice = _salePrice / _amount;
                }
            }
            
            // Update registry if we calculated a valid price
            if (newPrice > 0) {
                assetRegistry.updateSecondaryPrice(_assetId, newPrice);
            }
        } catch {
            // If we can't get the model or update fails, just continue
            // The sale was already successful, price update is optional
        }
    }
