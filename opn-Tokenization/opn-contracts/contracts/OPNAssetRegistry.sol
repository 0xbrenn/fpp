...
/**
 * @title OPNAssetRegistry - FIXED VERSION
...
contract OPNAssetRegistry is AccessControl, ReentrancyGuard, Pausable {
    ...
    // ✅ FIX: Add index mapping for O(1) admin removal
    ...
    constructor(address _usdcToken) {
        ...
        adminIndex[msg.sender] = 0; // ✅ FIX: Track index
    ...
    // ✅ FIX #3: Restrict asset deactivation to creator or super admin only
    function deactivateAsset(uint256 _assetId) external {
        Asset storage asset = assets[_assetId];
        require(asset.creator != address(0), "Asset not found");
        
        // Only creator or DEFAULT_ADMIN_ROLE can deactivate
        require(
    ...
    // ✅ FIX #5: Remove admin bypass - only position NFT can call
    ...
    // ✅ FIX #7: Optimized admin management with index tracking
    ...
    // ✅ FIX #7: O(1) removal using index mapping
