...
contract OPNAssetRegistry is AccessControl, ReentrancyGuard, Pausable {
    ...
    struct Asset {
        ...
        // Weighted Model
        uint256 totalValue;
        uint256 soldWeight; // in basis points (10000 = 100%)
    ...
    function createWeightedAsset(
        ...
        require(bytes(_assetName).length > 0, "Name required");
        require(_minPurchaseWeight > 0 && _minPurchaseWeight <= 10000, "Invalid min weight");
        require(_maxPurchaseWeight == 0 || _maxPurchaseWeight <= 10000, "Invalid max weight");
        ...
        a.minPurchaseAmount = _minPurchaseWeight; // stored in basis points
    ...
    function getAvailableSupply(uint256 _assetId) external view returns (uint256) {
        ...
        if (a.model == PurchaseModel.FIXED) {
            ...
            return 10000 - a.soldWeight; // basis points remaining
        }
    }
        ...
        external view returns (uint256) 
    {
        ...
        if (a.model == PurchaseModel.FIXED) {
            ...
            // _amount is in basis points (e.g., 50 = 0.5%)
            return (a.totalValue * _amount) / 10000;
        }
    }
