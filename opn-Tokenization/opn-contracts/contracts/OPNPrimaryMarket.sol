...
contract OPNPrimaryMarket is AccessControl, ReentrancyGuard, Pausable {
    ...
    function purchaseWeighted(uint256 _assetId, uint256 _weightBasisPoints) 
        external payable nonReentrant whenNotPaused 
    {
        // Get asset info using clean helper functions
        address creator = assetRegistry.getAssetCreator(_assetId);
        ...
        require(_weightBasisPoints > 0, "Invalid weight");
        require(_weightBasisPoints >= minPurchase, "Below minimum");
        ...
        if (maxPurchase > 0) {
            ...
            require(currentHoldings + _weightBasisPoints <= maxPurchase, "Exceeds maximum");
        }
        ...
        // Check available supply
        uint256 available = assetRegistry.getAvailableSupply(_assetId);
        require(_weightBasisPoints <= available, "Insufficient supply");
        
        // Calculate costs
        uint256 cost = assetRegistry.calculatePurchaseCost(_assetId, _weightBasisPoints);
        uint256 fee = (cost * platformFee) / 10000;
        uint256 totalCost = cost + fee;
        ...
        // Record sale in registry
        assetRegistry.recordSale(_assetId, _weightBasisPoints, cost);
        
        // Mint position NFT
        uint256 positionId = positionNFT.mintPosition(_assetId, msg.sender, _weightBasisPoints, cost);
        ...
        emit FractionsPurchased(_assetId, msg.sender, _weightBasisPoints, cost, fee, positionId);
    }
